...
while read coord; do
    ...
    EUR_N_count=$( cat /Genomics/akeylab/abwolf/NeanderthalSeq/IBDmix/Altai.2013.p_SingleArchaic.final_callset/Altai2013pub.1KGP3.LOD4.50K.filterAFRDeni.txt \
    | awk 'BEGIN {OFS="\t"} {if($1=='$chr' && $2>='$st' && $3<='$ed' && $8=="EUR") print $NF}' \
    | sort - | uniq - \
    | wc -l )

    # Randomly pick out the appropriate number of EUR/AFR samples to be reassigned as carrier/non-carrier
    shuf -n $AFR_N_count ALL_AFR.inds | sort - \
    > $dir/AFR_N.inds.${SLURM_ARRAY_TASK_ID}

    shuf -n $EUR_N_count ALL_EUR.inds | sort - \
    > $dir/EUR_N.inds.${SLURM_ARRAY_TASK_ID}
    ...
    comm ALL_EUR.inds EUR_N.inds.${SLURM_ARRAY_TASK_ID} \
    | cut -f 1 \
    | sort - | uniq - \
    | awk 'BEGIN {OFS="\t"} NR!=1 {print $0}' \
    > $dir/EUR_0.inds.${SLURM_ARRAY_TASK_ID}
    ...
    if [ $winst -lt $wined ]; then
        ...
        for i in N 0; do
            echo -n $chr $st $ed $winst $wined' ' >> $dir/AFR_"$i".EUR_"$i".results.perm.${SLURM_ARRAY_TASK_ID}
            vcftools \
            ...
            --weir-fst-pop EUR_"$i".inds.${SLURM_ARRAY_TASK_ID} \
            2>&1 >/dev/null \
            | grep mean | cut -f 7 -d ' ' >> $dir/AFR_"$i".EUR_"$i".results.perm.${SLURM_ARRAY_TASK_ID}
        done
    else
