...
dir=/Genomics/akeylab/abwolf/NeanderthalSeq/IBDmix/FSTtest/null/
...
while read coord; do
    ...
    if [ $winst -lt $wined ]; then
        echo $chr $st $ed
        for i in N 0; do
            echo -n $chr $st $ed $winst $wined' ' >> $dir/AFR_"$i".EUR_"$i".results.null
            vcftools \
            ...
            --to-bp $wined \
            --maf 0.05 \
            --weir-fst-pop $dir/AFR_"$i".inds \
            --weir-fst-pop $dir/EUR_"$i".inds \
            2>&1 >/dev/null \
            | grep mean | cut -f 7 -d ' ' >> $dir/AFR_"$i".EUR_"$i".results.null
        done
    else
