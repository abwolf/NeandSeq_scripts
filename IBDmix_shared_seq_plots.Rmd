...
p3 <- 
  ...
  scale_fill_brewer(type = 'qual', palette = 'Dark2') +
  ggtitle('Frequency of African Segments in African Population\n') +
  xlab('\nFrequency') +
...
p4 <- 
  ...
  geom_bar(data = data.table(c('EUR_AFR_Shared', 'EAS_AFR_Shared'), 
                             c(EUR_AFRshared.notEAS %>% mutate(len=end-strt) %>% summarise(sum(len)/1000000) %>% as.numeric(), 
                               EAS_AFRshared.notEUR %>%  mutate(len=end-strt) %>% summarise(sum(len)/1000000) %>% as.numeric() )
                             ) %>% 
                  setnames(c('category','sum_Mb')),
           aes(x=category, y=sum_Mb, fill=category), stat = "identity", width=0.5 ) +
  scale_x_discrete(limits=c('EAS_AFR_Shared', 'EUR_AFR_Shared'), labels=c('EAS_AFR', 'EUR_AFR')) +
...
print('generate graph')
#pdf("EmpiricalSegments_SFS.pdf", width = 30, height = 9)
...
#grid.arrange(p_0.001.a,p1,p3,p4, ncol=4)
dev.off()
...
nonAFR.total.unmerged <- fread('IntroSeg.EUR_ASN.D4E0.002.ALL.30kb.bed', col.names = c('chr','strt','end'))
...
AFR.nonAFRshared.unmerged.count <- fread('bedmap --ec --delim "\t" --echo --bases --fraction-map 0.001 --count IntroSeg.AFR.D4E0.002.ALL.30kb.bed IntroSeg.EUR_ASN.D4E0.002.ALL.30kb.bed', col.names = c('chr','strt','end', 'bases','count')) %>% filter(count>0)

nonAFR.AFRshared.unmerged.count <- fread('bedmap --ec --delim "\t" --echo --bases --fraction-map 0.001 --count IntroSeg.EUR_ASN.D4E0.002.ALL.30kb.bed IntroSeg.AFR.D4E0.002.ALL.30kb.bed', col.names = c('chr','strt','end', 'bases','count')) %>% filter(count>0)

nrow.AFR <- fread('IntroSeg.AFR.D4E0.002.ALL.30kb.bed') %>% nrow()  
nrow.EUR <- fread('IntroSeg.EUR.D4E0.002.ALL.30kb.bed') %>% nrow()
nrow.ASN <- fread('IntroSeg.ASN.D4E0.002.ALL.30kb.bed') %>% nrow()
nrow.nonAFR <- fread('IntroSeg.EUR_ASN.D4E0.002.ALL.30kb.bed') %>% nrow()
...
EUR_AFRshared.notEAS <- fread("bedmap --ec --delim '\t' --echo --bases --fraction-map 0.001 --indicator IntroSeg.EUR.D4E0.002.ALL.30kb.bed IntroSeg.AFR.D4E0.002.ALL.30kb.bed.merged | awk 'BEGIN {OFS=\"\t\"} {if ($NF==1) print $1,$2,$3 }' | bedmap --ec --delim '\t' --echo --fraction-map 0.001 --indicator - IntroSeg.ASN.D4E0.002.ALL.30kb.bed.merged | awk 'BEGIN {OFS=\"\t\"} { if($NF==0) print $1,$2,$3}' | bedmap --ec --delim '\t' --echo --count - ",
                                  col.names = c('chr','strt','end','count')) %>% filter(count>0)

EAS_AFRshared.notEUR <- fread("bedmap --ec --delim '\t' --echo --bases --fraction-map 0.001 --indicator IntroSeg.ASN.D4E0.002.ALL.30kb.bed IntroSeg.AFR.D4E0.002.ALL.30kb.bed.merged | awk 'BEGIN {OFS=\"\t\"} {if ($NF==1) print $1,$2,$3 }' | bedmap --ec --delim '\t' --echo --fraction-map 0.001 --indicator - IntroSeg.EUR.D4E0.002.ALL.30kb.bed.merged | awk 'BEGIN {OFS=\"\t\"} { if($NF==0) print $1,$2,$3}' | bedmap --ec --delim '\t' --echo --count - ",
                                  col.names = c('chr','strt','end','count')) %>% filter(count>0)
...
p4 <- 
  ...
  theme_classic() +
  geom_bar(data = data.table(c('EUR_AFR_Shared', 'EAS_AFR_Shared'), 
                             c(EUR_AFRshared.notEAS %>% mutate(len=end-strt) %>% summarise(sum(as.numeric(len))/1000000) %>% as.numeric(), 
                               EAS_AFRshared.notEUR %>%  mutate(len=end-strt) %>% summarise(sum(len)/1000000) %>% as.numeric() )
                             ) %>% 
                  setnames(c('category','sum_Mb')),
           aes(x=category, y=sum_Mb, fill=category), stat = "identity", width=0.5 ) +
...
print('generate graph')
# pdf("SimulatedSegments_LengthAndSFS.pdf", width = 30, height = 9)
#pdf("~/Documents/Dropbox/AkeyLab/Writing/IBDadmix_102018/Figures/n1_0.1_mAfB_0.0_mBAf_0.0_mAfEu_0.0_mEuAf_0.0_SegLengthSFS.pdf", width = 40, height = 9)
#pdf("~/Documents/Dropbox/AkeyLab/Writing/IBDadmix_102018/Figures/n1_0.1_mAfB_0.0_mBAf_0.0_mAfEu_0.0_mEuAf_0.00005_SegLengthSFS.pdf", width = 40, height = 9)
pdf("~/Documents/Dropbox/AkeyLab/Writing/IBDadmix_102018/Figures/n1_0.1_mAfB_0.0_mBAf_0.0_mAfEu_0.0_mEuAf_0.0005_SegLengthSFS.pdf", width = 40, height = 9)
#grid.arrange(p1, p3, ncol=2)
grid.arrange(p_0.001.a, p3, p4, ncol=3)
#grid.arrange(p_0.001.a, p1, p3, p4, ncol=4)
dev.off()
...
#### Simulated Data
  ...
  setwd(dirpath)
# Count of segments that overlap
s.AFR.EURshared.unmerged.count <- fread('bedmap --ec --delim "\t" --echo --bases --fraction-map 0.001 --count IntroSeg.AFR.D4E0.002.ALL.30kb.bed  IntroSeg.EUR.D4E0.002.ALL.30kb.bed', col.names = c('chr','strt','end','bases','count')) %>% filter(count>=0)

s.AFR.EASshared.unmerged.count <- fread('bedmap --ec --delim "\t" --echo --bases --fraction-map 0.001 --count IntroSeg.AFR.D4E0.002.ALL.30kb.bed IntroSeg.ASN.D4E0.002.ALL.30kb.bed', col.names = c('chr','strt','end','bases','count')) %>% filter(count>=0)

s.EUR.AFRshared.unmerged.count <- fread('bedmap --ec --delim "\t" --echo --bases --fraction-map 0.001 --count IntroSeg.EUR.D4E0.002.ALL.30kb.bed IntroSeg.AFR.D4E0.002.ALL.30kb.bed', col.names = c('chr','strt','end','bases','count')) %>% filter(count>=0)

s.EAS.AFRshared.unmerged.count <- fread('bedmap --ec --delim "\t" --echo --bases --fraction-map 0.001 --count IntroSeg.ASN.D4E0.002.ALL.30kb.bed IntroSeg.AFR.D4E0.002.ALL.30kb.bed', col.names = c('chr','strt','end','bases','count')) %>% filter(count>=0)
